...
router.post('/', (req, res) => {
    const { propietario, mascota, tipo_mascota, peso, sexo, id_medico, fecha, observaciones } = req.body;
    mysqlConnection.query(`INSERT INTO appointment(propietario, mascota, tipo_mascota, peso,sexo,id_medico,fecha,observaciones) 
        ...
        (err, rows, fields) => {
            if (!err) {
                res.status(201).json({ status: 'appointment  saved' });
            } else {
                console.log(err);
            }
        });
...
router.put('/', (req, res) => {
    ...
    mysqlConnection.query(`update appointment set username=?,password=?,nombres=?,apellidos=?,tipo_usuario=? where iduser=?`, [username, password, nombres, apellidos, tipo_usuario, iduser],
        (err, rows, fields) => {
            if (!err) {
                res.json({ status: 'appointment updated' });
            } else {
                console.log(err);
            }
        });
...
router.delete('/:id', (req, res) => {
    ...
    mysqlConnection.query(`delete from appointment where idappointment=?`, [id],
        (err, rows, fields) => {
            if (!err) {
                res.json({ status: 'Appointment deleted' });
            } else {
                console.log(err);
            }
        });
